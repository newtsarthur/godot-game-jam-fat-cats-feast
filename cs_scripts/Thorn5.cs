...
public partial class Thorn5 : Area2D
{
    ...
    private async void OnBodyEntered(Node2D body)
    {
        ...
        {
            _shouldFall = false;
            GD.Print("Thorn atingiu o chão/tilemapdddd");
            HasTouchedGround = true;
            var buttonTwo = GetNode<Button88>("../Button88");

            if (buttonTwo.MouseDeath != 0)
            {
                Node cutsceneNode = GetNode("../Cutscene");
    ...
    private async void OnCutsceneFinished()
    {
        ...
        GetTree().ChangeSceneToFile("res://scene6.tscn");
    }
